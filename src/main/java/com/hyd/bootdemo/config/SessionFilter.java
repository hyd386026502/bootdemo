...
public class SessionFilter implements Filter {
    ...
    private String[] urlFilters = new String[]{"/b","/c"};
    /**
       ...
       logger.info("过滤器");
        ...
        if (uri.equals("/b")){
           logger.info( ((HttpServletRequest)request).getContextPath());
            ((HttpServletResponse) response).sendRedirect(((HttpServletRequest) request).getContextPath()+"loggin.html");
        }else{
            chain.doFilter(httpRequest,httpResponse);
        }
      /*  if (requestType != null && requestType.endsWith("XMLHttpRequest")){
            response.getWriter().write("过滤le 哈哈");
        }else{
            ((HttpServletResponse) response).sendRedirect(((HttpServletRequest) request).getContextPath()+"loggin.html");
        }*/
    }
